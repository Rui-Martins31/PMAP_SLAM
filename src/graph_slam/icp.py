...
def welsch_weight(weights, p=0.5):
...
def icp(A, B, init_pose=None, max_iterations=20, tolerance=0.001):
    ...
    for i in range(max_iterations):
        ...
        # Reject pairs beyond max correspondence distance
        close_match_mask = distances < config.ICP_MAX_CORRESPONDENCE_DIST
        ...
        error_improvement = abs(prev_error - mean_error)
        
        if error_improvement < tolerance:
            break
            
        prev_error = mean_error

    # Compute final distances after all transformations applied
    final_distances, _ = nearest_neighbor(src.T, dst.T)
    return T_acc, final_distances, i